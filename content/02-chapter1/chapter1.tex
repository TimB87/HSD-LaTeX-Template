...
Zusätzlich ist die Option \glqq draft\grqq\ bei größeren Dokumenten hilfreich, denn es verarbeitet nicht alle Informationen und spart dadurch Zeit beim kompilieren. Diese Vorlage wird z. B. deshalb im ersten Durchlauf ebenfalls nur als \glqq draft\grqq-Variante kompiliert, da wir praktisch nur die Output-Dateien brauchen, die weiter als Input für \hologo{biber}, makeglossaries und xindy dienen.
...
\lstinputlisting[firstline=31,lastline=54,float=htb,caption=Der relevante Code für Header und Footer,label=lst:our-heafoo]{meta/preamble.tex}
...
Das Paket kann noch vieles mehr, zum Beispiel die korrekte Darstellung von Maßeinheiten:
...
\begin{lstlisting}[float=htb,caption=Einbindung einer Grafik so nah wie möglich am Text,label=lst:float_h]
  \begin{figure}[h!]
    \centering
       \includegraphics[]{ProdKonsRentemitSteuern.pdf}
    \caption{Die Besteuerung eines Marktes}
...
%}}}
\subsection{Zitieren}%{{{
Korrektes Zitieren und Kennzeichnen ist eine der heikelsten Angelegenheiten in der Thesis. Muss es aber gar nicht, denn dafür gibt es \hologo{BibTeX}.
...
\end{lstlisting}
Relevanter Code:
\lstinputlisting[firstline=147,lastline=168,float=htb,caption=Relevanter Code für biblatex,label=lst:our-bib]{meta/preamble.tex}
...
\lstinputlisting[firstline=10,lastline=22,float=htb,caption=arara regeln,label=lst:arara-rules]{main.tex}
...
\lstinputlisting[firstline=59,lastline=61,float=htb,caption=Unser Inhaltsverzeichnis,label=lst:ourtoc]{main.tex}
...
\lstinputlisting[firstline=171,lastline=180,float=htb,caption=Relevanter Code für das Abkürzungsverzeichnis,label=lst:our-acros]{meta/preamble.tex}
%}}}
...
\lstinputlisting[firstline=182,lastline=187,float=htb,caption=Relevanter Code für unser Glossar,label=lst:our-glo]{meta/preamble.tex}
...
\begin{lstlisting}[float=htb,caption=Das Setzen von Indextoken mit \protect\LaTeX{},label=lst:indextoken]
Schreiben sie von \index{Alpha}Alpha bis \index{Omega}Omega.
\end{lstlisting}
