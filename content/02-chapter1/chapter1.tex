...
\lstinputlisting[language=TeX,firstline=23,lastline=46,float=htpb,caption=Der relevante Code für Header und Footer,label=lst:our-heafoo]{meta/preamble.tex}
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Darstellung von Zahlen in \protect\LaTeX{},label=lst:zahlen]
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Darstellungen von Einheiten mit siunitx,label=lst:einheiten]
\si{\kilo\gram\metre\per\square\second} \\
\si{\gram\per\cubic\centi\metre}
\\
\si{\square\volt\cubic\lumen\per\farad} \\
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Darstellungen von mathematischen Formeln,label=lst:inlinemath]
$ SEW\footnote{Summe der Endwerte} (RF\footnote{Rückflüsse}) = \sum_{t=1}^{n} [ ( E_{t} - A_{t}) \times (1+i)\ ^{N-t}] $
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Darstellungen von Mathe mit der \glqq displayed math\grqq-Methode,label=lst:dspmath]
\[
  \varphi = \frac{1 + \sqrt{5}}{2} = 1.618 \ldots
\]
\end{lstlisting}
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Darstellungen einer Tabelle in \protect\LaTeX{},label=lst:bsptabelle]
\begin{table}[htbp]
\begin{tabular}{|l|l|l|l|l|l|l|}
\hline
t & 0 & 1 & 2 & 3 & 4 & 5 \\ \hline
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Einbindung einer Grafik so nah wie möglich am Text,label=lst:float_h]
  \begin{figure}[h!]
    \centering
       \includegraphics[]{ProdKonsRentemitSteuern.pdf}
    \caption{Die Besteuerung eines Marktes}
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Automatische Einbindung einer Grafik,label=lst:float_htbp]
  \begin{figure}[htbp]
    \centering
       \includegraphics[width=1\linewidth]{PMSteuer.pdf}
    \caption{Steueraufkommen}
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Das setzen von Fußnoten mit \protect\LaTeX{},label=lst:footnotes]
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Das Setzen von Zitations \protect\LaTeX{},label=lst:cites]
Es sei zu erwähnen, das biber\footcite[][]{kime_biber_2019} ebenfalls ein eigenes Handbuch hat, dass weiterhelfen kann!
\end{lstlisting}
Relevanter Code:
\lstinputlisting[language=TeX,firstline=135,lastline=156,float=htpb,caption=Relevanter Code für biblatex,label=lst:our-bib]{meta/preamble.tex}
...
\lstinputlisting[language=TeX,firstline=4,lastline=14,float=htpb,caption=arara regeln,label=lst:arara-rules]{main.tex}
...
\lstinputlisting[language=TeX,firstline=52,lastline=54,float=htpb,caption=Unser Inhaltsverzeichnis,label=lst:ourtoc]{main.tex}
...
\lstinputlisting[language=TeX,firstline=159,lastline=168,float=htpb,caption=Relavanter Code für das Abkürzungsverzeichnis,label=lst:our-acros]{meta/preamble.tex}
%}}}
...
\lstinputlisting[language=TeX,firstline=170,lastline=175,float=htpb,caption=Relevanter Code für unser Glossar,label=lst:our-glo]{meta/preamble.tex}
...
\begin{lstlisting}[language=TeX,float=htpb,caption=Das Setzen von Indextoken mit \protect\LaTeX{},label=lst:indextoken]
Schreiben sie von \index{Alpha}Alpha bis \index{Omega}Omega.
\end{lstlisting}

Mit einem Index kann man Schlagworte und Themengruppen zusammenfassen und dem Leser helfen, diese im Dokument zu finden.

\lstinputlisting[language=TeX,firstline=177,lastline=180,float=htpb,caption=Relevanter Code für unseren Index,label=lst:our-idx]{meta/preamble.tex}%}}}
%}}}
